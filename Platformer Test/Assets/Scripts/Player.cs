...
public class Player : MonoBehaviour {
    ...
    void Update () {
        ...
        if (!inputDisabled)
        {
            ...
            // Dash function
            // If there was no second input after initial one within time, disable dash
            ...
            // if the first input is taken and the second input is taken within time, and dash is not on cooldown, make the character dash for .33 seconds
            ...
            // while dashing, speed and max speed is multiplied by 2.5 second. Puts dash on cooldown for 4 seconds.
            if (dashing)
            {
            ...
            // while not dashing, take in initial input and set input timer.
            // disables dash on the opposite key so that player cannot dash by pressing left, right, left in quick succession. In other words, player can only dash by pessing right, right or left, left
            else if (InputManager.RightKeyDown())
            {
                dashBoolRight = true;
                dashRightInputTimer = 0.25f;
                dashLeftInputTimer = 0;
            }
            else if (InputManager.LeftKeyDown())
            {
                dashBoolLeft = true;
                dashLeftInputTimer = 0.25f;
            ...
            // if the duration is less than or equal to 0, the character is not dashing and reset speed modifiers
            else
            {
                dashing = false;
                maxSpeedX = 3;
    ...
    void FixedUpdate()
    {
        ...
        rb2d.AddForce((Vector2.right * speed) * h * dashSpeedModifier);
